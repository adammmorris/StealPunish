...
%% Params
% x: [lr, temp, gamma, numSubj steal biases, numSubj punisher biases]
...
function [likelihood] = getIndivLike_SP_v2(subjMarkers, x, roles, opponents, myActions, rewards1, rewards2, memory)
...
steal_bias = zeros(numSubj,1);
for i = 1:numSubj
    steal_bias(i) = x(3+i);
end

punish_bias = zeros(numSubj,1);
for i = 1:numSubj
    steal_bias(i) = x(3+numSubj+i);
end
...
for thisSubj = 1:numSubj
    if thisSubj < length(subjMarkers)
        ...
        index = subjMarkers(thisSubj):length(roles);
    end
    ...
    for thisRound = index
        ...
        if role == ROLE_THIEF
            ...
            reward1 = reward1 + steal_bias(thisSubj)*(myAction==ACTION_S);
        ...
        if role == ROLE_PUNISHER
            ...
            reward2 = reward2 + punish_bias(thisSubj)*(myAction==ACTION_P & lastActions(1)==ACTION_S);
