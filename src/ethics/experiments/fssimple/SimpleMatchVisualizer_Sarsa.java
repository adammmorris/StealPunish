...
import domain.stocasticgames.foragesteal.simple.FSSimpleJAM_Sym;
import domain.stocasticgames.foragesteal.simple.FSSimpleJR;
import domain.stocasticgames.foragesteal.simple.FSSimplePOJR;
import domain.stocasticgames.foragesteal.simple.FSSimplePOJR_Sym;
import domain.stocasticgames.foragesteal.simple.FSSimple_Sym;
...
import ethics.experiments.fssimple.specialagents.MatchAnalyzer_Sarsa;
import ethics.experiments.fssimple.specialagents.SarsaAgent;
import ethics.experiments.fssimple.specialagents.SarsaAgentFactory;
...
public class SimpleMatchVisualizer_Sarsa extends JFrame {
	...
	public SimpleMatchVisualizer(){
		...
		//FSSimple_Sym dgen = new FSSimple_Sym(3);
		...
		JointActionModel jam = new FSSimpleBTSJAM(backTurnedProb, backTurnedProb);
		//JointActionModel jam = new FSSimpleJAM_Sym();
		
		//this.objectiveRF = new FSSimpleJR();
		//this.objectiveRF = new FSSimpleJR(1., -0.5, -2.5, 0.);
		this.objectiveRF = new FSSimplePOJR(1., -0.5, -2.5, 0.);
		//this.objectiveRF = new FSSimplePOJR_Sym(1., -0.5, -2.5, 0.);
	...
	protected void playMatch(){
		...
		baseAgentFactory = new SarsaAgentFactory(domain,discount,learningRate,1.5,hashingFactory,lambda,new TBForageStealFAbstraction());
	...
	protected void performMatch(double [] a0SRParams, double [] a1SRParams, JointReward[] subjectiveRFStorage){
		...
		SarsaAgent agent0 = (SarsaAgent)a0Factory.generateAgent();
		...
		SarsaAgent agent1 = (SarsaAgent)a1Factory.generateAgent();
	...
	protected MatchAnalyzer_Sarsa getMatchAnalyzer(World w, SarsaAgent agent0, SarsaAgent agent1){
		
		MatchAnalyzer_Sarsa ma = new MatchAnalyzer_Sarsa(w, agent0, agent1, extractStatesFromQuery(agent0QueryStates), extractStatesFromQuery(agent1QueryStates));
		ma.setMaxStages(this.maxStage);
		return ma;
	...
	protected List<StateActionSetTuple> getQueryStates(String [] agentNames, int playerId){
		...
		if(playerId == 0){
			...
			State s0 = FSSimple_Sym.getInitialState(this.domain, agentNames[0], agentNames[1], 0, 0);
			...
			State s1 = FSSimple_Sym.getInitialState(this.domain, agentNames[0], agentNames[1], 0, 0);
			FSSimple_Sym.setStateNode(s1, 1);
			...
			State s2 = FSSimple_Sym.getInitialState(this.domain, agentNames[0], agentNames[1], 0, 0);
			FSSimple_Sym.setStateNode(s2, 2);
			...
			State s3 = FSSimple_Sym.getInitialState(this.domain, agentNames[0], agentNames[1], 0, 0);
			FSSimple_Sym.setStateNode(s3, 3);
			StateActionSetTuple sas3 = new StateActionSetTuple(s3, "S3");
			sas3.addAction(new GroundedSingleAction(agentNames[playerId], this.domain.getSingleAction(FSSimple.ACTIONPUNISH), ""), "P");
			sas3.addAction(new GroundedSingleAction(agentNames[playerId], this.domain.getSingleAction(FSSimple.ACTIONDONOTHING), ""), "N");
			res.add(sas3);
	...
	public static void main(String[] args) {
		SimpleMatchVisualizer_Sarsa smv = new SimpleMatchVisualizer_Sarsa();
		smv.initGUI();
		smv.pack();
		smv.setVisible(true);
