 ...
 *
 */
public class SarsaAgent extends Agent implements QComputablePlanner{
	...
	// Last outcome stuff
	protected State last_s;
	protected JointAction last_ja;
	protected Map<String,Double> last_jr;
	protected State last_sprime;
	 ...
	 */
	public SarsaAgent(SGDomain d, double discount, double learningRate, double defaultQ, StateHashFactory hashFactory, double lambda) {
		this.init(d);
	...
	public void observeOutcome(State s, JointAction jointAction, Map<String, Double> jointReward, State sprime, boolean isTerminal) {
		...
		if (!this.firstTurn) {
			// Do update for last turn
			GroundedSingleAction last_action = this.last_ja.action(worldAgentName);
			QValue last_qv = this.getQ(this.last_s, last_action);
			
			last_qv.q = last_qv.q + this.learningRate.pollLearningRate(this.last_s, last_action) * this.lambda * (r + (this.discount * maxQ) - qv.q);
		}
		
		this.saveLastOutcome(s, jointAction, jointReward, sprime);
	}
	...
	protected void saveLastOutcome(State s, JointAction jointAction, Map<String, Double> jointReward, State sprime) {
		this.last_s = s;
		this.last_ja = jointAction;
		this.last_jr = jointReward;
		this.last_sprime = sprime;
