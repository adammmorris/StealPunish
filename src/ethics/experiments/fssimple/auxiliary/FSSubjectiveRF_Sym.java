...
public class FSSubjectiveRF_Sym implements ParameterizedRF{

	protected JointReward		objectiveRewardFunction;
	protected double [] 		params; //0 is steal bias 1 is punish bias
	
	
	public FSSubjectiveRF_Sym(JointReward objectiveRewardFunction){
	...
	public Map<String, Double> reward(State s, JointAction ja, State sp) {
		...
		for(GroundedSingleAction gsa : ja){
			...
			if(gsa.action.actionName.equals(FSSimple_Sym.ACTIONSTEAL)){
				nr += this.params[0];
			}
			else if(gsa.action.actionName.equals(FSSimple_Sym.ACTIONPUNISH)){
				nr += this.params[1];
			}
